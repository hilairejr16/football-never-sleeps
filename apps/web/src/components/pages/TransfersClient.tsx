...
const STATUS_TABS = ['All', 'Confirmed', 'Rumours', 'Completed'] as const;
...
export default function TransfersClient() {
  ...
  const [count, setCount] = useState(BATCH * 2);

  const filtered = tab === 'All'
    ? SUMMER_2026_TRANSFERS
    : SUMMER_2026_TRANSFERS.filter(t =>
        tab === 'Confirmed' ? t.status === 'confirmed'
        : tab === 'Rumours'   ? t.status === 'rumour'
        : t.status === 'completed'
      );
    ...
    <div className="max-w-screen-2xl mx-auto px-4 lg:px-6 py-8">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3 mb-6">
        {STATUS_TABS.map(t => (
          <button
            key={t}
            onClick={() => { setTab(t); setCount(BATCH * 2); }}
          ...
          >
            ...
            {t !== 'All' && (
              <span className="ml-1.5 text-[10px] opacity-60">
                ({SUMMER_2026_TRANSFERS.filter(x =>
                  t === 'Confirmed' ? x.status === 'confirmed'
                  : t === 'Rumours' ? x.status === 'rumour'
                  : x.status === 'completed'
                ).length})
              </span>
            )}
      ...
      {visible.length === 0 ? (
        <div className="gr-card p-10 text-center text-brand-gray">
          No {tab.toLowerCase()} transfers to show yet.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-6">
          {visible.map(t => <TransferCard key={t.id} transfer={t}/>)}
        </div>
      )}
